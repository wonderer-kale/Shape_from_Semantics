...
class Dream3DOrthogonalIterableDataset(RandomCameraIterableDataset):
    ...
    def obtain_view_near_condition(self, batch):
        # sample elevation angles
        elevation_deg: Float[Tensor, "B"]
        elevation: Float[Tensor, "B"]
        ...
        sampled_view = random.choice(self.cfg.views)
        elevation_deg = sampled_view["elevation"] + torch.rand(self.batch_size) * 50 - 25 # FIXME: hard-coded variation range
        elevation = elevation_deg * math.pi / 180
        azimuth_deg = sampled_view["azimuth"] + torch.rand(self.batch_size) * 100 - 50 # FIXME: hard-coded variation range
        azimuth = azimuth_deg * math.pi / 180
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        singleview_cond = self.obtain_view_near_condition(batch)

        return singleview_cond
