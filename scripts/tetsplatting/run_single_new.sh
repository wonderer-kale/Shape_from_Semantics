...
python3 launch.py --config configs/nd-mv-tetsplatting/geo.yaml \
    --train --gpu "$GPUS" \
    system.prompt_processor.prompts="$prompts" \
    system.prompt_processor.elevations="$elevations" \
    system.prompt_processor.azimuths="$azimuths" \
    system.nd_prompt_processor.prompts="$prompts" \
    system.nd_prompt_processor.elevations="$elevations" \
    system.nd_prompt_processor.azimuths="$azimuths" \
    use_timestamp=False \
    name="$geo_out" \
...
python3 launch.py --config configs/nd-mv-tetsplatting/tex.yaml \
    ...
    system.prompt_processor.elevations="$elevations" \
    system.prompt_processor.azimuths="$azimuths" \
    system.albedo_prompt_processor.prompts="$prompts" \
    system.albedo_prompt_processor.elevations="$elevations" \
    system.albedo_prompt_processor.azimuths="$azimuths" \
